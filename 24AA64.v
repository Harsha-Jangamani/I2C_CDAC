...
// *******************************************************************************************************
   ...
   initial begin
      `ifdef VCC_1_7V_TO_2_5V
         ...
         tWC = 1000000;             // memory write cycle time
      `else
...
// -------------------------------------------------------------------------------------------------------
   ...
   always @(negedge SCL) begin
      if (START_Rcvd & (BitCounter == 8)) begin
         if (!WriteActive & (ShiftRegister[07:01] == {4'b1100,ChipAddress[02:00]})) begin
   ...
   always @(negedge SCL) begin
      if (!WriteActive) begin
         if (BitCounter == 8) begin
            if (WrCycle | (START_Rcvd & (ShiftRegister[07:01] == {4'b1100,ChipAddress[02:00]}))) begin
               SDA_DO <= 0;
               SDA_OE <= 1;
               #10;
            end 
...
// -------------------------------------------------------------------------------------------------------
   ...
   always @(SDA_DriveEnable) SDA_DriveEnableDlyd <= #(0) SDA_DriveEnable;
